...
function App() {
  ...
  const addItem = () => {
    ...
    if (existingItemIndex !== -1) {
      ...
      const updatedDataItems = dataItems.map((dataItem, index) => {
        if (index === existingItemIndex) {
          return {
            ...dataItem,
            qty: dataItem.qty + newItem.qty,
            discount: dataItem.discount + newItem.discount,
          };
        }
        return dataItem;
      });
        ...
        <Col md={4} style={{ backgroundColor: "#e4e4e4" }}>
          ...
          <Row>
            <Col>
              ...
              <Form.Control type="number" ref={discountRef} value={discount} onChange={(e) => setDiscount(discountRef.current.value)} />
            </Col>
          </Row>
          <hr />
